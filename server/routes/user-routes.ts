...
/**
 * Get user's progression state
 */
router.get('/progression-state', requireAuth, async (req, res) => {
  try {
    ...
    // Parse navigation progress to get progression state
    ...
    if (result.user.navigationProgress) {
      try {
        const navProgress = JSON.parse(result.user.navigationProgress);
        progressionState = {
          ...progressionState,
          ...navProgress
        };
    ...
    res.json({
      success: true,
      data: progressionState
    });
  } catch (error) {
    console.error('Error getting progression state:', error);
    res.status(500).json({
      success: false,
      error: 'Failed to load progression state'
    });
  }
});

/**
 * Update user's progression state
 */
router.post('/progression-state', requireAuth, async (req, res) => {
  try {
    const userId = req.session.userId;
    if (!userId) {
      return res.status(401).json({
    ...
    // Save progression state as navigation progress
    const navigationProgress = JSON.stringify(progressionData);
    ...
    if (!result.success) {
      return res.status(400).json({
        success: false,
        error: result.error || 'Failed to update progression state'
      });
    }

    res.json({
      success: true,
      message: 'Progression state updated successfully'
    });
  } catch (error) {
    console.error('Error updating progression state:', error);
    res.status(500).json({
      success: false,
      error: 'Failed to save progression state'
    });
  }
});
