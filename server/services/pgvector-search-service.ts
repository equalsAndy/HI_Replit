 ...
 * ================================
 * Improved training document retrieval using PostgreSQL full-text search + pgvector infrastructure
 * This provides semantic search capabilities without requiring OpenAI embeddings initially
 */
...
export class PgvectorSearchService {
  ...
  async searchTrainingDocuments(
    ...
    try {
      ...
      // Create search terms that prioritize relevant documents
      const searchTerms = [
        query,
        reportType,
        'strengths signature',
   ...
   */
  async getOptimalTrainingPrompt(
    reportType: 'personal' | 'professional',
    userContext: {
      name: string;
      strengths: any;
    ...
    try {
      ...
      // NEW APPROACH: Use OpenAI Vector Database Integration (same as admin screen)
      // Instead of sending full training documents, reference them in OpenAI's vector store
...
Generate the complete report now using your vector database knowledge and this user's data.`;
   ...
   */
  private extractPromptSections(fullPrompt: string, sections: string[]): string {
   ...
   */
  private async buildUserContextPrompt(userContext: any, reportType: string): Promise<string> {
    try {
      // Get personalization documents from database
      ...
      // Calculate user-specific data for template substitution
      ...
      // Calculate total strengths percentages
      const totalStrengths = strengthEntries.reduce((sum, [_, value]) => sum + Number(value), 0);
      const strengthPercentages = strengthEntries.map(([key, value]) => 
        `${key}: ${((Number(value) / totalStrengths) * 100).toFixed(1)}%`
      ).join(', ');

      // Get second highest strength
      ...
      // Build personalized context using document-based instructions  
      return `
...
`;
      ...
      // Fallback to simpler approach if document retrieval fails
   ...
   */
  private buildSimpleUserContext(userContext: any, reportType: string): string {
    const strengths = userContext.strengths || {};
    const strengthEntries = Object.entries(strengths).map(([key, value]) => [key, Number(value)]);
    const dominantStrength = strengthEntries.length > 0 ? 
      strengthEntries.reduce((a, b) => (a[1] > b[1] ? a : b))[0] : 'unknown';
...
`;
   ...
   */
  private getFallbackPrompt(reportType: string): string {
...
Generate the complete report immediately without explanation.`;
   ...
   */
  async getExampleReports(reportType: 'personal' | 'professional'): Promise<string> {
    try {
      const result = await pool.query(`
        SELECT content 
        FROM training_documents 
      ...
      if (result.rows.length > 0) {
        // Return a focused excerpt of the example
