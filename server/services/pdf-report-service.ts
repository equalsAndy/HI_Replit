...
export class PDFReportService implements PDFGenerationService {
  ...
  async generatePDF(reportData: HolisticReportData, starCardImagePath: string): Promise<Buffer> {
    ...
    try {
      ...
      // Get StarCard image - prioritize data from report, fallback to file path
      let starCardImageBase64 = '';
      
      // Check if report data includes StarCard data directly
      if ((reportData as any).starCardData?.photoData) {
        starCardImageBase64 = (reportData as any).starCardData.photoData;
        console.log('📊 Using StarCard data from report');
      } else if (starCardImagePath) {
        // Fallback to converting file path to base64
      ...
      // Generate pie chart for strengths visualization
...
<body>
  ...
  private formatAIContent(content: string): string {
    // Format AI content as proper HTML paragraphs, not markdown
  ...
  private formatSectionContent(content: string): string {
    ...
    return paragraphs.map(paragraph => {
      ...
      // Handle bullet points - convert to professional insights
      if (trimmed.includes('\n- ') || trimmed.startsWith('- ')) {
        const items = trimmed.split(/\n?- /).filter(item => item.trim()).map(item => item.trim());
        return `
      ...
      // Handle quotes and highlighted text
      if (trimmed.startsWith('"') && trimmed.endsWith('"')) {
        return `<blockquote class="report-quote">${trimmed.slice(1, -1)}</blockquote>`;
      }
