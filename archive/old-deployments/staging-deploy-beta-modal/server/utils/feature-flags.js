...
export function isFeatureEnabled(featureName, environment) {
...
export function validateFlagConfiguration() {
    const errors = [];
    const checkCircularDeps = (flagName, visited = new Set()) => {
...
export function getAIFeatures() {
    return Object.fromEntries(Object.entries(featureFlags).filter(([_, flag]) => flag.aiRelated));
}
export function getFlagsByEnvironment(environment) {
    return Object.fromEntries(Object.entries(featureFlags).map(([name]) => [
        name,
