...
export class BetaTesterNotesService {
    static async createNote(note) {
        try {
            const result = await pool.query(`INSERT INTO beta_tester_notes (
    ...
    static async getUserNotes(userId, workshopType, includeSubmitted = false) {
        try {
            let query = `
        SELECT 
    ...
    static async updateNote(noteId, userId, noteContent) {
        try {
            const result = await pool.query(`UPDATE beta_tester_notes 
         SET note_content = $1, updated_at = CURRENT_TIMESTAMP
         WHERE id = $2 AND user_id = $3 AND is_deleted = false`, [noteContent, noteId, userId]);
    ...
    static async updateNoteAdvanced(noteId, userId, updates) {
        try {
            const setClause = [];
            const params = [];
            let paramIndex = 1;
            ...
            const result = await pool.query(`UPDATE beta_tester_notes 
         SET ${setClause.join(', ')}
         WHERE id = $${paramIndex++} AND user_id = $${paramIndex++}`, params);
            return result.rowCount > 0;
        }
        catch (error) {
            console.error('❌ Error updating beta tester note:', error);
    ...
    static async submitAllUserNotes(userId, workshopType) {
        try {
            let query = `
    ...
    static async getUserNotesSummary(userId, workshopType) {
        try {
            let query = `
        SELECT 
    ...
    static async submitFinalFeedback(feedbackData) {
        try {
            const result = await pool.query(`INSERT INTO beta_feedback_surveys (
    ...
    static async checkFeedbackStatus(userId) {
        try {
            const result = await pool.query(`SELECT id, submitted_at 
