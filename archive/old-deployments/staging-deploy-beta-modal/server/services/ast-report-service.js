...
class ASTReportService {
    async generateASTReports(userData) {
        try {
    ...
    analyzeStrengthsConstellation(strengths) {
    ...
    getConstellationArchetype(primary, secondary) {
        const archetypes = {
    ...
    async generatePersonalReport(userData, signature, flowCategory, context) {
...
${Object.entries(userData.stepReflections).map(([step, reflection]) => `${step}: "${reflection}"`).join('\n')}
...
This is a CONFIDENTIAL personal development report for ${userData.userName}'s private use only.`;
    ...
    async generateProfessionalProfile(userData, signature, flowCategory, context) {
...
This is a PROFESSIONAL profile report suitable for sharing with colleagues and team members to optimize collaboration with ${userData.userName}.`;
    ...
    async getUserASTData(userId) {
      ...
      `, [userId]);
            ...
            if (reflectionsData.reflections?.strength1)
                stepReflections['2-4-1'] = reflectionsData.reflections.strength1;
            if (reflectionsData.reflections?.strength2)
                stepReflections['2-4-2'] = reflectionsData.reflections.strength2;
            if (reflectionsData.reflections?.strength3)
                stepReflections['2-4-3'] = reflectionsData.reflections.strength3;
            if (reflectionsData.reflections?.strength4)
                stepReflections['2-4-4'] = reflectionsData.reflections.strength4;
            if (reflectionsData.reflections?.teamValues)
                stepReflections['team-values'] = reflectionsData.reflections.teamValues;
            if (reflectionsData.reflections?.uniqueContribution)
                stepReflections['unique-contribution'] = reflectionsData.reflections.uniqueContribution;
            if (futureData.twentyYearVision)
                stepReflections['4-4'] = futureData.twentyYearVision;
            if (futureData.tenYearMilestone)
                stepReflections['4-4-10yr'] = futureData.tenYearMilestone;
            if (futureData.fiveYearFoundation)
                stepReflections['4-4-5yr'] = futureData.fiveYearFoundation;
            if (futureData.flowOptimizedLife)
                stepReflections['4-5'] = futureData.flowOptimizedLife;
            if (roundingOutData.values)
                stepReflections['flow-triggers'] = roundingOutData.values;
            if (roundingOutData.strengths)
                stepReflections['flow-blockers'] = roundingOutData.strengths;
            if (roundingOutData.passions)
                stepReflections['flow-conditions'] = roundingOutData.passions;
            if (roundingOutData.growthAreas)
                stepReflections['flow-improvements'] = roundingOutData.growthAreas;
            return {
    ...
    async saveReports(userId, reports) {
        try {
            await pool.query(`
