export class EnhancedReportPrompting {
    generateReportPrompt(userData, reportType) {
...
Begin the report now:`;
    ...
    analyzeStarCardSignature(starCard) {
    ...
    formatStarCardData(starCard, analysis) {
        return `
...
This creates a unique operating pattern where ${analysis.dominant.name.toLowerCase()} provides the primary energy flow, supported by ${analysis.secondary.name.toLowerCase()} capabilities.`;
    ...
    formatAssessmentInsights(insights) {
        if (insights.length === 0)
            return 'Standard assessment profile completed.';
        return insights.map((insight, index) => `${index + 1}. ${insight}`).join('\n');
    }
    formatPersonalReflections(reflections) {
        if (!reflections)
            return 'No detailed reflections available.';
    ...
    formatFutureVision(futureVision) {
        let formatted = '';
...
Future Target Level: ${wellBeingData.futureWellBeingLevel}/10`;
    ...
    generateProfessionalPrompt(userData) {
