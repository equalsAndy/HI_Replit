...
router.get('/users', requireAuth, isFacilitatorOrAdmin, async (req, res) => {
    try {
...
router.post('/users', requireAuth, isAdmin, async (req, res) => {
    try {
...
router.get('/invites', requireAuth, isFacilitatorOrAdmin, async (req, res) => {
    try {
        const userRole = req.session.userRole;
        const userId = req.session.userId;
        let invitesResult;
...
router.post('/invites/batch', requireAuth, isAdmin, async (req, res) => {
    try {
...
router.put('/users/:id', requireAuth, isAdmin, async (req, res) => {
    try {
        ...
        if (updateData.resetPassword) {
            processedUpdateData.password = undefined;
        }
...
router.put('/users/:id/role', requireAuth, isAdmin, async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        if (isNaN(id)) {
            return res.status(400).json({ message: 'Invalid user ID' });
...
router.put('/users/:id/test-status', requireAuth, isAdmin, async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        if (isNaN(id)) {
            return res.status(400).json({ message: 'Invalid user ID' });
...
router.get('/test-users', requireAuth, isAdmin, async (req, res) => {
    try {
...
router.get('/beta-testers', requireAuth, isAdmin, async (req, res) => {
    try {
...
router.get('/videos', requireAuth, isAdmin, async (req, res) => {
    try {
...
router.get('/videos/workshop/:workshopType', requireAuth, isAdmin, async (req, res) => {
    try {
...
router.put('/videos/:id', requireAuth, isAdmin, async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        if (isNaN(id)) {
...
router.get('/users/:userId/export', requireAuth, async (req, res) => {
    try {
...
router.get('/users/:userId/validate', requireAuth, isAdmin, async (req, res) => {
    try {
        const userId = parseInt(req.params.userId);
        if (isNaN(userId)) {
            return res.status(400).json({
