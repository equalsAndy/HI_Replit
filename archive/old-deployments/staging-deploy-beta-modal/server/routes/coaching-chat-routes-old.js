...
router.post('/message', authenticateUser, async (req, res) => {
    try {
        ...
        const aiResponse = await generateCoachingResponse(message, conversation.conversationType, context, userData, conversation.demoMode);
...
router.get('/conversation/:conversationId', authenticateUser, async (req, res) => {
    try {
        ...
        const [conversation] = await db
            .select()
            .from(coachingConversations)
            .where(and(eq(coachingConversations.id, conversationId), eq(coachingConversations.userId, userId)))
            .limit(1);
        if (!conversation) {
            return res.status(404).json({ error: 'Conversation not found' });
        }
...
async function getUserWorkshopContext(userId) {
    try {
        const [user] = await db
            .select()
            .from(users)
...
async function generateCoachingResponse(userMessage, conversationType, context, userData, demoMode) {
    try {
        let systemPrompt;
...
async function generateStructuredResponse(userMessage, conversationType, context, demoMode) {
