...
router.get('/config', requireAdmin, async (req, res) => {
    try {
        const result = await pool.query('SELECT * FROM ai_configuration ORDER BY feature_name');
        res.json({
            success: true,
...
router.get('/usage/stats', requireAdmin, async (req, res) => {
    try {
        ...
        const statsResult = await pool.query('SELECT * FROM ai_usage_statistics WHERE hour >= NOW() - INTERVAL \'$1 hours\' ORDER BY hour DESC, feature_name', [hoursNum]);
...
router.post('/emergency-disable', requireAdmin, async (req, res) => {
    try {
        ...
        await pool.query('UPDATE ai_configuration SET enabled = false, updated_at = NOW()');
...
router.get('/training-docs/:id', requireAdmin, async (req, res) => {
    const { id } = req.params;
    try {
        const docResult = await pool.query('SELECT * FROM training_documents WHERE id = $1', [id]);
...
router.delete('/training-docs/:id', requireAdmin, async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query('DELETE FROM training_documents WHERE id = $1 RETURNING original_filename', [id]);
        if (result.rows.length === 0) {
...
router.get('/personas', requireAdmin, async (req, res) => {
    try {
        ...
        const filteredPersonas = CURRENT_PERSONAS.filter(persona => persona.environments.includes(currentEnvironment));
...
router.post('/beta-testers/:userId', requireAdmin, async (req, res) => {
    ...
    try {
        if (grant) {
            await pool.query('UPDATE users SET is_beta_tester = true, beta_tester_granted_at = NOW(), beta_tester_granted_by = $1 WHERE id = $2', [adminUserId, userId]);
            ...
            await pool.query('UPDATE users SET is_beta_tester = false, beta_tester_granted_at = NULL, beta_tester_granted_by = NULL WHERE id = $1', [userId]);
...
router.get('/training-docs/:id/content', requireAdmin, async (req, res) => {
    const { id } = req.params;
    try {
        const result = await pool.query('SELECT id, title, content FROM training_documents WHERE id = $1', [id]);
        if (result.rows.length === 0) {
            return res.status(404).json({
                success: false,
                error: 'Training document not found'
...
router.put('/training-docs/:id', requireAdmin, async (req, res) => {
    ...
    try {
        ...
        const updateResult = await pool.query('UPDATE training_documents SET content = $1, updated_at = NOW() WHERE id = $2 RETURNING title, document_type', [content, id]);
        ...
        for (let i = 0; i < chunks.length; i++) {
            ...
            await pool.query(`INSERT INTO document_chunks (
          document_id, chunk_index, content, token_count, metadata
        ) VALUES ($1, $2, $3, $4, $5)`, [id, i, chunk.content, chunk.tokenCount, JSON.stringify({
