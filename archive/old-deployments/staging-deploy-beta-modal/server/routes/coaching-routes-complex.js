...
import { coachKnowledgeBase, userProfilesExtended, coachingSessions, connectionSuggestions } from '../../shared/schema.js';
...
router.get('/knowledge-base', requireAuth, async (req, res) => {
    try {
        ...
        if (category) {
            query = db.select().from(coachKnowledgeBase).where(eq(coachKnowledgeBase.category, category));
        }
...
router.post('/profile/extended', requireAuth, async (req, res) => {
    try {
        ...
        const { company, department, role, expertiseAreas, projectExperience, collaborationPreferences, availabilityStatus, connectionOptIn } = req.body;
...
router.post('/session', requireAuth, async (req, res) => {
    try {
        const userId = req.session.userId;
        const { conversation, sessionSummary, contextUsed, sessionType, sessionLength, userSatisfaction } = req.body;
...
router.post('/connection-suggestion', requireAuth, async (req, res) => {
    try {
        ...
        const { suggestedCollaboratorId, reasonType, reasonExplanation, context } = req.body;
