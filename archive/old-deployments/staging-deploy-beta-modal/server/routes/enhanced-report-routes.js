...
function generateExecutiveSummaryContent(starCard, stepByStep, userName, percentages) {
...
function generateStrengthsTableContent(percentages, stepByStep) {
...
function generateFlowOptimizationContent(flow, flowAttributes) {
    ...
    <div class="flow-content">
      ...
      <ul>
        ...
        <li><strong>Optimal Conditions:</strong> ${(attributes || []).map((attr) => String(attr?.name || '')).join(', ') || 'Structured environment with clear objectives'}</li>
      </ul>
      ...
      <div class="ai-suggestion">
        ...
        <h4>AI Suggestion:</h4>
        <p>Schedule critical and creative tasks during optimal focus periods; minimize interruptions during deep work; leverage top attributes (${(attributes || []).slice(0, 2).map((attr) => String(attr?.name || '')).join(', ')}) to structure challenging projects.</p>
      </div>
    </div>
  `;
}
function generateWellbeingContent(cantril) {
...
function generateFutureVisionContent(futureSelf, cantril) {
    return `
...
function generateConstraintsContent(roundingOut, stepByStep, percentages) {
...
function generateTeamSynergyContent(stepByStep, percentages) {
    return `
...
function generateReflectionContent(finalReflection, roundingOut) {
    return `
...
function generateAIRecommendationsContent(percentages, flow, userName) {
    return `
