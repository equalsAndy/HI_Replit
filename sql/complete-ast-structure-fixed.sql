...
DELETE FROM videos WHERE workshop_type = 'allstarteams';
...
INSERT INTO videos (
  title, description, url, editable_id, workshop_type, section, step_id, autoplay, sort_order,
  transcript_md, glossary, created_at, updated_at
) VALUES (
  'Flow Patterns',
...
SELECT 
  ...
  sort_order,
  CASE 
    WHEN transcript_md IS NOT NULL THEN 'Has Transcript (' || LENGTH(transcript_md) || ' chars)'
    ELSE 'No Transcript' 
