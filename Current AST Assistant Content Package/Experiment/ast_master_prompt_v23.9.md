# 🧭 Master Prompt – AST Personal Development Report (v23.9)
...
- Each flow attribute may indirectly connect to one or more strengths domains (Thinking, Acting, Feeling, Planning).  
...
**v23.8 Updates:**
- Ensures `<visual>` syntax replaces all `<rml:>` formats.  
- Ladder visuals now require explicit `id="wellbeing_ladder"`.  
- Vision tag support added (`vision1–4`), with placement and symbolic interpretation.  
- Added policy for symbolic description of visuals (especially user-uploaded ones).  
- Expanded guidance for contextual narrative image interpretation.  
- Examples clarified for assistant comprehension, not user output.  
- Aligned all tone, statelessness, and RML compliance rules with v12+ instruction sets.
