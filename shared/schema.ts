...
// IA Workshop Types and Validation
export interface IACoreCabilitiesData {
  imagination: number;
  curiosity: number;
  empathy: number;
  creativity: number;
...
export interface NavigationProgress {
  ...
  videoProgress: Record<string, number | { farthest: number; current: number }>;
}

// IA Step ID Validation
export const IAStepIdSchema = z.string().regex(/^ia-\d+-\d+$/, 'IA Step ID must follow format: ia-X-Y');

// Validate IA step IDs
export function isValidIAStepId(stepId: string): boolean {
  return /^ia-\d+-\d+$/.test(stepId);
}

// App type validation
export const AppTypeSchema = z.enum(['ast', 'ia']);

// Assessment type validation with IA support
export const AssessmentTypeSchema = z.enum([
  ...
  'flowAttributes', 
  'cantrilLadder',
  'iaCoreCabilities' // New IA assessment type
]);