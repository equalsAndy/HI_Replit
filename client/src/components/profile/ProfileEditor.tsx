...
export default function ProfileEditor({ user, onLogout, currentApp, onToggleWorkshop }: ProfileEditorProps) {
  ...
  const getUserInitials = (nameOrFirst?: string, last?: string) => {
    if (last !== undefined) {
      // Called with firstName, lastName
      return ((nameOrFirst?.[0] || '') + (last?.[0] || '')).toUpperCase() || 'U';
    }
    // Called with a full name string
    return (nameOrFirst || '')
      .split(' ')
      .map(n => n[0])
      .join('')
        ...
        <Button variant="ghost" size="sm" className="flex items-center gap-2">
          <Avatar className="h-6 w-6">
            ...
            <AvatarFallback className="text-xs">
              {user?.firstName ? getUserInitials(user.firstName, user.lastName) : <User className="h-3 w-3" />}
            </AvatarFallback>
          </Avatar>
          ...
          <div className="flex flex-col items-center space-y-4">
            <Avatar className="h-24 w-24">
              ...
              <AvatarFallback className="text-lg">
                {formData.firstName ? getUserInitials(formData.firstName, formData.lastName) : <User className="h-8 w-8" />}
              </AvatarFallback>
            </Avatar>
            ...
            {isEditing ? (
              ...
              <>
                <div>
                  ...
                  <p className="text-sm mt-1 p-2 bg-muted rounded-md">{formData.firstName ? `${formData.firstName}${formData.lastName ? ' ' + formData.lastName : ''}` : 'Not provided'}</p>
                </div>
