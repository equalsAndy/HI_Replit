...
const ImaginationAssessmentContent = ({ navigate, markStepCompleted, setCurrentContent }: ImaginationAssessmentContentProps) => {
  ...
  const { data: existingAssessment, isLoading } = useQuery({
    queryKey: ['/api/assessments', 'imaginationAssessment'],
    enabled: true
  });
  ...
  useEffect(() => {
    if (existingAssessment?.assessment?.results) {
      setScores(existingAssessment.assessment.results);
      setShowResults(true);
    }
  }, [existingAssessment]);
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="flex justify-center space-x-4">
          ...
          <Button 
            onClick={() => setCurrentContent('insights-review')}
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-8 mb-8">
        <div className="space-y-6">
          ...
          <div className="flex items-center space-x-3">
            <div className={`p-2 rounded-lg ${categoryColors[currentQ.category as keyof typeof categoryColors]} text-white`}>
              {currentQ.icon}
            </div>
            <span className={`px-3 py-1 rounded-full text-sm font-medium ${categoryColors[currentQ.category as keyof typeof categoryColors]} text-white`}>
              {currentQ.category.charAt(0).toUpperCase() + currentQ.category.slice(1)}
            </span>
            {currentQ.subcategory && (
              <span className="text-sm text-gray-500">
                • Sub-dimension: {currentQ.subcategory}
              </span>
            )}
          </div>
...
export default ImaginationAssessmentContent;